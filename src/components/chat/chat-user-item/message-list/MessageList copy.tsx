        ...
        <div style={{ position: "relative", height: "100%" }}>
            ...
            <Virtuoso
                ...
                itemContent={(index, messageItem: TAllmessage) => {
                    const isSender = messageItem.userIdSender === user?.id;

                    const memberSender = stateChat.chatGroupMembers.find((m) => m.userId === messageItem.userIdSender);

                    // const memberRecipient = stateChat.chatGroupMembers.find((m) => m.userId !== user?.id);
                    ...
                    return (
                        <Fragment key={index}>
                            {isSender ? (
                                <SenderMessageItem
                                    messageItem={{
                                        avatar: user?.avatar ?? memberSender?.avatar,
                                        fullName: user?.fullName ?? memberSender?.fullName,
                                ...
                                <RecipientMessageItem
                                    messageItem={{
                                        avatar: memberSender?.avatar,
                                        fullName: memberSender?.fullName,
                                        message: messageItem.messageText,
                                        createdAt: messageItem.createdAt || "",
